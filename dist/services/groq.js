...
exports.buildSettingsBlock = buildSettingsBlock;
exports.buildSystemPrompt = buildSystemPrompt;
exports.buildSystemPromptForChat = buildSystemPromptForChat;
...
async function loadProjectSettings(projectId) {
    ...
    let characters = [];
    try {
        characters = await prisma_1.prisma.character.findMany({
            where: { projectId },
...
function buildSettingsBlock(args) {
    ...
    if (project.coreConflict)
        lines.push(`Core conflict: ${project.coreConflict}`);
    ...
    if (world?.length) {
        ...
        for (const w of world.slice(0, 12)) {
            const body = w.summary ?? JSON.stringify(w.traits ?? {});
            lines.push(`- ${w.name}: ${body}`);
        }
    }
    return lines.join('\n');
}
