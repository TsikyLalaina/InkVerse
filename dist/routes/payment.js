...
const paymentRoutes = async (app) => {
    ...
    app.post('/payment/webhooks/stripe', {
        config: {
        ...
        if (event.type === 'checkout.session.completed') {
            ...
            if (userId && type === 'coin_purchase') {
                ...
                if (amountTotal > 0)
                    coinsToAdd = 100; // Placeholder logic
                // Update User Balance
    ...
    // Get Wallet Balance
    app.get('/payment/balance', { preHandler: [app.auth] }, async (req, reply) => {
        const user = req.user;
        if (!user)
            return reply.status(401).send({ error: 'Unauthorized' });
    ...
    // Unlock Chapter
    app.post('/payment/unlock-chapter', { preHandler: [app.auth] }, async (req, reply) => {
        const { chapterId, cost } = req.body;
        const user = req.user;
        if (!user)
            return reply.status(401).send({ error: 'Unauthorized' });
        // 1. Check if already unlocked
        const existing = await prisma_1.prisma.unlockedContent.findUnique({
            where: {
                userId_chapterId: {
                    userId: user.id,
                    chapterId: chapterId,
        ...
        await prisma_1.prisma.$transaction([
            // Update Balance
            prisma_1.prisma.userProfile.update({
                where: { userId: user.id },
                data: {
            ...
            prisma_1.prisma.unlockedContent.create({
                data: {
                    userId: user.id,
                    chapterId: chapterId,
    ...
    app.get('/payment/author-stats', { preHandler: [app.auth] }, async (req, reply) => {
        const user = req.user;
        if (!user)
            return reply.status(401).send({ error: 'Unauthorized' });
        // 1. Calculate Total Earnings from Unlocked Content
        // We need to join Chapter -> Project -> User
        const earnings = await prisma_1.prisma.unlockedContent.groupBy({
        ...
        const unlocks = await prisma_1.prisma.unlockedContent.findMany({
    ...
    // ADMIN: List Payouts
    app.get('/payment/admin/payouts', { preHandler: [app.auth] }, async (req, reply) => {
        const user = req.user;
        if (!user)
            return reply.status(401).send({ error: 'Unauthorized' });
        // Verify Admin Access
        const adminRecord = await prisma_1.prisma.admin.findUnique({
            where: { userId: user.id }
        });
        if (!adminRecord) {
    ...
    app.post('/payment/admin/payouts/:id/approve', { preHandler: [app.auth] }, async (req, reply) => {
        const user = req.user;
        if (!user)
            return reply.status(401).send({ error: 'Unauthorized' });
        // Verify Admin Access
        ...
        const { id } = req.params;
        const payout = await prisma_1.prisma.payoutRequest.update({
            where: { id },
    ...
    // ADMIN: Reject Payout
    app.post('/payment/admin/payouts/:id/reject', { preHandler: [app.auth] }, async (req, reply) => {
        const user = req.user;
        if (!user)
            return reply.status(401).send({ error: 'Unauthorized' });
        // Verify Admin Access
    ...
    app.post('/payment/request-payout', { preHandler: [app.auth] }, async (req, reply) => {
        ...
        const projects = await prisma_1.prisma.project.findMany({ where: { userId: user.id }, select: { id: true } });
        const chapterIds = (await prisma_1.prisma.chapter.findMany({ where: { projectId: { in: projects.map(p => p.id) } }, select: { id: true } })).map(c => c.id);
        const unlocks = await prisma_1.prisma.unlockedContent.findMany({ where: { chapterId: { in: chapterIds } }, select: { paidPrice: true } });
        const totalCoins = unlocks.reduce((acc, u) => acc + u.paidPrice, 0);
        const COIN_VALUE_USD = 0.005;
        const totalRevenue = Math.floor(totalCoins * COIN_VALUE_USD * 100);
        const payouts = await prisma_1.prisma.payoutRequest.findMany({ where: { userId: user.id } });
        const used = payouts.filter(p => p.status !== 'rejected').reduce((acc, p) => acc + p.amount, 0);
        const available = totalRevenue - used;
        const request = await prisma_1.prisma.payoutRequest.create({
            data: {
                userId: user.id,
                amount,
                coinsDeducted: 0,
