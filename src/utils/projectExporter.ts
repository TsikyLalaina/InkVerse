 ...
 */
export async function exportProjectAsJson(projectId: string, userId: string): Promise<ExportedProject> {
  try {
    ...
    const worldSettings = await (prisma as any).worldSetting.findMany({
      where: { projectId },
      orderBy: { createdAt: 'asc' },
      select: {
        name: true,
    ...
    return {
      ...
      mode: (project.mode as any) || 'novel',
      ...
      worldSettings: worldSettings.map((ws: any) => ({
        name: ws.name,
 ...
 */
export async function exportProjectAsMarkdown(projectId: string, userId: string): Promise<string> {
  try {
    ...
    if (data.characters.length > 0) {
      ...
      data.characters.forEach((char) => {
        ...
        if (char.summary) markdown += `${char.summary}\n`;
        markdown += `\n`;
      });
    }
    ...
    if (data.worldSettings.length > 0) {
      ...
      data.worldSettings.forEach((ws) => {
        markdown += `### ${ws.name}\n`;
        if (ws.summary) markdown += `${ws.summary}\n`;
        markdown += `\n`;
      });
    }
 ...
 */
export async function exportProjectAsText(projectId: string, userId: string): Promise<string> {
  try {
    const data = await exportProjectAsJson(projectId, userId);
    ...
    if (data.characters.length > 0) {
      ...
      data.characters.forEach((char) => {
        text += `${char.name}\n`;
        if (char.role) text += `  Role: ${char.role}\n`;
        if (char.summary) text += `  ${char.summary}\n`;
        text += `\n`;
      });
    }
