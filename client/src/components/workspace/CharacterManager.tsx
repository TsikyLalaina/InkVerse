...
export function CharacterManager({ projectId, requireImage }: { projectId: string; requireImage: boolean }) {
  ...
  const addChild = (pathToObj: Array<string|number>, depth: number) => {
    ...
    setTraits((curr: any) => {
      const nxt = clone(curr);
      // Find target object and its parent so we can replace to control key order
      let parent: any = null;
      let ref: any = nxt;
      for (let i = 0; i < pathToObj.length; i++) {
        parent = ref;
        ref = ref[pathToObj[i]];
      }
      if (!ref || typeof ref !== 'object' || Array.isArray(ref)) return nxt;
  ...
  return (
    <div className="h-full grid grid-rows-[auto_1fr]">
      ...
      <div className="min-h-0 grid" style={{ gridTemplateColumns: isMobile ? "1fr" : (isTablet ? "40% 60%" : "35% 65%") }}>
        <aside className={classNames(isMobile && mobileView==='detail' && 'hidden', "border-b md:border-r border-border-default min-h-0 flex flex-col") }>
          <div className="p-3">
            <div className="relative">
              <input value={filter} onChange={(e) => setFilter(e.target.value)} placeholder="Search characters" className="w-full bg-bg-primary border border-border-default rounded-md pl-9 pr-3 py-3 md:py-2.5 text-base md:text-sm min-h-[44px] text-text-primary placeholder:text-text-tertiary outline-none focus:border-accent" />
          ...
          <div ref={listRef} className="flex-1 overflow-y-auto px-3 pb-3">
            ...
            {filtered.map((c) => (
              <button key={c.id} onClick={() => { if (isMobile) { setListScrollTop(listRef.current?.scrollTop || 0); setMobileView('detail'); } setActiveId(c.id!); }} className={classNames("w-full text-left px-3 py-3 min-h-[44px] rounded-md mb-2 transition-all", activeId===c.id ? "border-l-2 border-blue-500 bg-bg-elevated" : "hover:bg-bg-hover") }>
                <div className="flex items-center gap-3">
        ...
        <section className={classNames(isMobile && mobileView==='list' && 'hidden', "min-h-0 overflow-y-auto p-6")} aria-label="Character details">
          {activeId ? (
            <div className="max-w-2xl mx-auto">
              ...
              <div className="grid gap-4">
                ...
                <label className="grid gap-2 text-sm">
                  ...
                  <input value={form.role || ""} onChange={(e)=>setForm((f)=>({ ...f, role: e.target.value }))} list="roles" className="w-full bg-bg-primary border border-border-default rounded-md px-4 py-3 md:py-2.5 text-base md:text-sm min-h-[48px] text-text-primary outline-none focus:border-accent" />
                  <datalist id="roles">
